...
def ball_collision(ball):
    if ball.x - BALL_RADIUS <= 0 or ball.x + BALL_RADIUS >= WIDHT:
...
def main():
    ...
    def display_text(text):
        ...
        win.blit(text_render, (WIDHT/2 - text_render.get_width()/2, HEIGHT/2 - text_render.get_height()/2))
        pygame.display.update()
    ...
    while run:
        ...
        if keys[pygame.K_LEFT] and paddle.x - paddle.VEL >= 0:
            paddle.move(-1)
        if keys[pygame.K_RIGHT]and paddle.x + paddle.width + paddle.VEL <= WIDHT:
            paddle.move(1)
